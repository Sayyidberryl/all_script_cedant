...
_SUSPEND_JOIN_COLS = [
    ...
    "insured_ori", "clean insured 1", "clean insured 2", "clean insured 3",
    "clean insured 4", "clean insured 5", "clean insured 6", "clean insured 7",
    "clean insured 8", "clean insured 9", "INSURED_ORI", "INSURED_1", "INSURED_2",
    "CURR ORI", "CURR PAY", "AMOUNT PAY",
    "polis_ori", "clean polis 1", "POLIS", "POLIS_CLN",
    "clean sertif 1", "SERTIF_CLN",
    "slip_ori", "clean slip 1", "SLIP_NO", "SLIP_NO_CLN",
...
def _build_output_row(
    ...
    return {
        ...
        "POLIS":             suspend_row.get("POLIS") or suspend_row.get("polis_ori", ""),
        "POLIS_CLN":         suspend_row.get("POLIS_CLN") or suspend_row.get("clean polis 1", ""),
        "SERTIF_CLN_1":      _format_sertif(suspend_row.get("SERTIF_CLN_1") or suspend_row.get("SERTIF_CLN") or suspend_row.get("clean sertif 1", "")),
        ...
        "SLIP_NO":           suspend_row.get("SLIP_NO") or suspend_row.get("slip_ori", ""),
        "SLIP_NO_CLN":       suspend_row.get("SLIP_NO_CLN") or suspend_row.get("clean slip 1", ""),
...
def run() -> None:
    """Pipeline utama V2: matching suspend vs OSBAL/SLIPDB/FACUL, output diakumulasi (merge rows)."""
    t_start = time.perf_counter()
    print("\n" + "=" * 60, flush=True)
    print("  PRODUCTION SCRIPT — SUSPEND MATCHING  (ACA) — V2 (AKUMULASI)", flush=True)
    print("=" * 60, flush=True)
    ...
    # [4c/6] Fac code claim & accumulation (V2: merge rows)
    ...
    try:
        ...
        df.to_sql("SUSPENSE_DATA_SUSPENSE_V2", con=engine, if_exists="append", index=False)
        print(f"      -> {len(df):,} rows berhasil di-export ke 'SUSPENSE_DATA_SUSPENSE_V2'")
    except Exception as e:
