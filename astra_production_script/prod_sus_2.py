...
OUTPUT_FILE  = os.path.join("data", "final_output_astrabuana_v2.xlsx")
...
# Kolom CLSDT / FAC prioritas di Suspend (tidak ada di data Astra Buana → fallback otomatis)
...
FINAL_COLUMNS = [
    ...
    "FLAG_PROD",
    "POLIS", "POLIS_CLN",
    "SERTIF_CLN_1", "SERTIF_CLN_2", "SERTIF_CLN_3",  # no sertifikat bersih 1-3
    "SLIP_NO", "SLIP_NO_CLN",
    "DESC 1", "DESC 2", "DESC 3", "DESC 4",
    "STATUS", "REC_TYPE",
    "SKENARIO",
...
def _get_clean_cols(columns: list, prefix: str) -> list:
    ...
    alt_map = {
        ...
        "fac_policy_no_clean_": ["fac_policy_no_clean_", "policy_clean_", "polis_clean_", "clean polis "],
        "fac_policy_clean_": ["fac_policy_clean_", "policy_clean_", "polis_clean_", "clean polis "],
...
def _narrow(
    ...
    """Narrow matched candidates using the complementary fields cascading: Polis -> Slip -> Insured."""
    ...
    if len(current_matched) > 1 and ("POLIS" in label or "SLIP" in label):
        confirmed_ins = [i for i in current_matched if _ref_has_value(rows[i], insured_ref_cols, "insured_ori", insured_values)]
        if confirmed_ins:
            current_matched = confirmed_ins if len(current_matched) > 1 else current_matched
...
def _narrow_by_currency(
    ...
    return filtered  # STRICT: eliminasi jika beda currency
...
def _narrow_by_periode(
    ...
    return valid  # STRICT: eliminasi jika beda periode
...
def _match_polis_sertif(
    ...
    if not sertif_vals:
        # Tidak ada sertif di suspend row → serahkan ke Pass 1 (Polis/Slip)
        return [], "Unmatching"
    ...
    # Sertif tidak cocok → serahkan ke Pass 1 (Polis/Slip)
    return [], "Unmatching"
...
def _compute_derived_cols(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Tanpa toleransi: nilai harus benar-benar sama persis (hanya mentoleransi float precision < 1e-5)
    is_equal = (amount_neg - balance_due).abs() < 1e-5
...
def run() -> None:
    ...
    bordero_cols               = []
    if bordero_files:
        print(f"  Loading {len(bordero_files)} Bordero file(s) (FIRE, MC, MH) ...", flush=True)
    ...
    # [2] Detect clean columns (prefix spesifik per tabel — Astra Buana)
    ...
    # Tambahkan CLSDT ke OSBAL/FACUL jika ada
    ...
    # Bangun sertif index untuk OSBAL
    ...
    for n, sus in enumerate(suspend_rows, 1):
        ...
        # Dijalankan HANYA jika ada sertif di baris suspend dan OSBAL punya kolom sertif.
        if sertif_sus and sertif_osbal and _collect_clean_values(sus, sertif_sus):
        ...
        # Pass 3: SUSPEND → FACUL → OSBAL resolution
        ...
        # Pass 6: Insured fallback → FACUL → OSBAL
        if not matched and facul_rows:
    ...
    # [4b/5] Fac code accumulation (post-matching)
    print("\n[4b/5] Fac code accumulation ...", flush=True)
    _EXCLUDED_SCENARIOS = {"Beda Currency", "Beda Periode"}
    _EXCLUDED_SCENARIOS_FULL = _EXCLUDED_SCENARIOS

    claimed: set = set()
    for r in raw_results:
        if len(r["fac_codes"]) == 1 and r["scenario"] not in _EXCLUDED_SCENARIOS_FULL:
            claimed |= r["fac_codes"]

    for r in raw_results:
        if len(r["fac_codes"]) > 1:
            remaining = r["fac_codes"] - claimed
            if len(remaining) == 1 and r["scenario"] not in _EXCLUDED_SCENARIOS_FULL:
    ...
    # [4c/5] Bordero narrowing (Open Cover FIRE, MC, MH)
    print("\n[4c/5] Bordero narrowing (Open Cover FIRE, MC, MH) ...", flush=True)
    ...
    if bordero_idx:
        ...
        if mc_period_narrowed:
            print(f"  Baris disempitkan oleh bordero MC period : {mc_period_narrowed:,}", flush=True)
        if amount_net_narrowed:
            print(f"  Baris disempitkan oleh bordero amount NET : {amount_net_narrowed:,}", flush=True)
    else:
        print("  Bordero: tidak tersedia, narrowing bordero dilewati", flush=True)

    # [5/5] Build output DataFrame
    ...
    try:
        ...
        print(f"  [WARN] '{OUTPUT_FILE}' terkunci/sedang dibuka di Excel. Menyimpan ke '{save_path}' ...", flush=True)
        df.to_excel(save_path, index=False)
